...
/**
 * A lock fixes length, not position. Squared walls share normal displacement;
 * locked walls share both displacements. Propagate through connected groups
 * so the far end of a locked wall and its neighbouring run move together.
 */
export function propagateSquaredWallEdit(
  baseline: WallDragWall[], proposed: WallDragWall[],
  anchor: { wallId: string; pointIndex: number },
): WallDragWall[] {
  if (!baseline.some((wall) => Object.keys(wall.lengthOverridesMm ?? {}).length > 0)) return proposed;
  const result = proposed.map((wall) => ({ ...wall, points: wall.points.map((point) => ({ ...point })) }));
  const nodes = baseline.flatMap((wall) => wall.points.map((point, pointIndex) => ({ wall, pointIndex, point })));
  const indices = new Map(nodes.map((node, index) => [`${node.wall.id}:${node.pointIndex}`, index]));
  const resultById = new Map(result.map((wall) => [wall.id, wall]));
  const baselineById = new Map(baseline.map((wall) => [wall.id, wall]));
  const anchorIndex = indices.get(`${anchor.wallId}:${anchor.pointIndex}`);
  for (const axis of ["x", "y"] as const) {
    ...
    const join = (first: number, second: number) => { parents[root(second)] = root(first); };
    nodes.forEach((node, index) => {
      const end = node.wall.points[node.pointIndex + 1];
      const nextIndex = indices.get(`${node.wall.id}:${node.pointIndex + 1}`);
      const lock = node.wall.lengthOverridesMm?.[node.pointIndex];
      if (end && nextIndex !== undefined && (
        (typeof lock === "number" && Number.isFinite(lock) && lock > 0)
        || Math.abs(end[axis] - node.point[axis]) <= ORTHOGONAL_TOLERANCE_MM
      )) join(index, nextIndex);
      // Include shared junctions and the duplicate closing vertex.
      for (let other = 0; other < index; other += 1) {
        if (samePoint(node.point, nodes[other].point, ORTHOGONAL_TOLERANCE_MM)) join(index, other);
      }
      const attachment = node.wall.attachments?.[node.pointIndex];
      if (!attachment) return;
      const host = baselineById.get(attachment.wallId);
      const start = host?.points[attachment.segmentIndex];
      const finish = host?.points[attachment.segmentIndex + 1];
      if (!host || !start || !finish) return;
      const atStart = attachment.along <= 0.000001;
      const atEnd = attachment.along >= 0.999999;
      if (atStart || atEnd || Math.abs(finish[axis] - start[axis]) <= ORTHOGONAL_TOLERANCE_MM) {
        const hostIndex = indices.get(`${host.id}:${attachment.segmentIndex + (atEnd ? 1 : 0)}`);
        if (hostIndex !== undefined) join(index, hostIndex);
      }
    });
    const deltas = new Map<number, number>();
    nodes.forEach((node, index) => {
      const target = resultById.get(node.wall.id)?.points[node.pointIndex];
      if (!target) return;
      const delta = target[axis] - node.point[axis];
      const group = root(index);
      if (!deltas.has(group) || Math.abs(delta) > Math.abs(deltas.get(group)!)) deltas.set(group, delta);
    });
    if (anchorIndex !== undefined) {
      const node = nodes[anchorIndex];
      const target = resultById.get(node.wall.id)?.points[node.pointIndex];
      if (target) deltas.set(root(anchorIndex), target[axis] - node.point[axis]);
    }
    nodes.forEach((node, index) => {
      const target = resultById.get(node.wall.id)?.points[node.pointIndex];
      if (target) target[axis] = node.point[axis] + (deltas.get(root(index)) ?? 0);
    });
  }
  return result;
}
...
/**
 * Remove one user-entered wall-length lock while leaving the current geometry
 * untouched. Subsequent corner or wall drags can then resize that segment.
 */
export function releaseWallLengthOverride(walls: WallDragWall[], wallId: string, segmentIndex: number): WallDragWall[] {
  return walls.map((wall) => {
    if (wall.id !== wallId || wall.lengthOverridesMm?.[segmentIndex] === undefined) return wall;
    const lengthOverridesMm = { ...wall.lengthOverridesMm };
    delete lengthOverridesMm[segmentIndex];
    return { ...wall, lengthOverridesMm: Object.keys(lengthOverridesMm).length ? lengthOverridesMm : undefined };
  });
}
...
export function constrainTranslatedWallDistance(
  ...
  for (const wall of baselineWalls) {
    wall.points.slice(0, -1).forEach((candidateStart, candidateSegmentIndex) => {
      ...
      const movedWall = candidateWalls?.find((item) => item.id === wall.id);
      const movedStart = movedWall?.points[candidateSegmentIndex];
      const movedEnd = movedWall?.points[candidateSegmentIndex + 1];
      // Connected locked runs translate together, so their separation stays
      // constant. Their old positions must not act as stationary obstacles.
      if (movedStart && movedEnd
        && Math.abs((movedStart.x - candidateStart.x) * normal.x + (movedStart.y - candidateStart.y) * normal.y - requestedDistance) <= ORTHOGONAL_TOLERANCE_MM
        && Math.abs((movedEnd.x - candidateEnd.x) * normal.x + (movedEnd.y - candidateEnd.y) * normal.y - requestedDistance) <= ORTHOGONAL_TOLERANCE_MM) return;
...
export function constrainSquaredCornerTarget(
  ...
  wall.points.slice(0, -1).forEach((start, segmentIndex) => {
    ...
    const distance = constrainTranslatedWallDistance(baselineWalls, wallId, segmentIndex, requestedDistance, minimumClearance, candidateWalls);
  ...
  baselineWalls.forEach((connectedWall) => {
    ...
    connectedWall.points.slice(0, -1).forEach((start, segmentIndex) => {
      ...
      const distance = constrainTranslatedWallDistance(baselineWalls, connectedWall.id, segmentIndex, requestedDistance, minimumClearance, candidateWalls);
      const correction = distance - requestedDistance;
      const axis = Math.abs(normal.x) >= Math.abs(normal.y) ? "x" : "y";
      axisTargets[axis].push(requestedTarget[axis] + normal[axis] * correction);
    });
