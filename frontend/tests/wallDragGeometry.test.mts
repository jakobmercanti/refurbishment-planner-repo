...
test("releases one manual wall-length lock without changing its geometry", () => {
  const walls: WallDragWall[] = [
    { id: "room", points: [{ x: 0, y: 0 }, { x: 1200, y: 0 }], lengthOverridesMm: { 0: 1200, 1: 900 } },
    { id: "other", points: [{ x: 0, y: 900 }, { x: 800, y: 900 }], lengthOverridesMm: { 0: 800 } },
  ];
  const released = releaseWallLengthOverride(walls, "room", 0);

  assert.equal(released[0].lengthOverridesMm?.[0], undefined);
  assert.equal(released[0].lengthOverridesMm?.[1], 900);
  assert.deepEqual(released[0].points, walls[0].points);
  assert.deepEqual(released[1], walls[1]);
});
...
test("accepts a manual notch-wall length by moving the safe endpoint", () => {
  const points = [
    { x: 0, y: 0 }, { x: 785, y: 0 }, { x: 785, y: 1000 }, { x: 3878, y: 1000 },
    { x: 3878, y: 2000 }, { x: 1000, y: 2000 }, { x: 1000, y: 2730 }, { x: 0, y: 2730 }, { x: 0, y: 0 },
  ];
  const enforced = enforceWallLengthOverridesPreservingOrthogonality([{
    id: "notched-outline",
    points,
    lengthOverridesMm: { 6: 1000 },
  }]);

  assert.ok(enforced);
  assert.deepEqual(enforced?.[0].points[6], { x: 1000, y: 2730 });
  assert.deepEqual(enforced?.[0].points[7], { x: 0, y: 2730 });
  assert.deepEqual(enforced?.[0].points.at(-1), enforced?.[0].points[0]);
});
