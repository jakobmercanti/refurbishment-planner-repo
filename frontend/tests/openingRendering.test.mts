...
import { openingRenderWidths, outwardNormalForRoom } from "../lib/openingRendering.ts";
...
test("projecting window normals always point away from the room", () => {
  const right = { x: 1, y: 0 };
  assert.deepEqual(outwardNormalForRoom(right, { x: 0, y: 100 }, { x: 500, y: 100 }), { x: -1, y: 0 });
  assert.deepEqual(outwardNormalForRoom(right, { x: 0, y: 100 }, { x: -500, y: 100 }), right);
  assert.deepEqual(outwardNormalForRoom(right, { x: 0, y: 100 }), right);
});
