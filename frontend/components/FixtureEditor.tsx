...
export function FixtureEditor({ room, displayUnits, onChange }: FixtureEditorProps) {
    ...
    <section className="fixture-editor" aria-label="Bathroom element editor">
      ...
      <div className="fixture-field-group">
        ...
        <div className="fixture-fields three-columns">
          <label className="field"><span>X <small>{UNIT_LABEL[displayUnits]}</small></span><DisplayNumberInput valueMm={x} units={displayUnits} onMmChange={setX} /></label>
          <label className="field"><span>Y <small>{UNIT_LABEL[displayUnits]}</small></span><DisplayNumberInput valueMm={y} units={displayUnits} onMmChange={setY} /></label>
      ...
      <div className="fixture-field-group">
        ...
        <div className="fixture-fields three-columns">
          <label className="field"><span>Width <small>{UNIT_LABEL[displayUnits]}</small></span><DisplayNumberInput minMm={1} valueMm={width} units={displayUnits} onMmChange={setWidth} /></label>
          <label className="field"><span>Depth <small>{UNIT_LABEL[displayUnits]}</small></span><DisplayNumberInput minMm={1} valueMm={depth} units={displayUnits} onMmChange={setDepth} /></label>
          <label className="field"><span>Height <small>{UNIT_LABEL[displayUnits]}</small></span><DisplayNumberInput minMm={1} valueMm={height} units={displayUnits} onMmChange={setHeight} /></label>
        </div>
      </div>
          ...
          {fixtures.map((fixture) => {
            ...
            return (
              <article key={fixture.id} className={editingId === fixture.id ? "editing" : ""}>
                ...
                <div><strong>{fixture.name}</strong><p>{formatLength(fixture.dimensions.width.value, displayUnits)} × {formatLength(fixture.dimensions.depth.value, displayUnits)} · X {formatLength(fixture.center.x, displayUnits, 0)}, Y {formatLength(fixture.center.y, displayUnits, 0)}{fixture.wall_lock ? " · Wall locked" : ""}</p></div>
