...
export function AnnotationsPanel({ tool, style, selection, onToolChange, onStyleChange, onTextChange, onDeleteSelected, measurementMode, onAddMeasurement, onRemoveMeasurement, onClearToolSelection }: AnnotationsPanelProps) {
  return <div className="annotations-panel" onPointerDown={(event) => {
    ...
    <div className="annotation-panel-group annotation-style-group">
      ...
      <label className="annotation-style-row"><span>Thickness</span><select value={style.thickness} onChange={(event) => onStyleChange({ thickness: Number(event.target.value) })}><option value="1">Thin - 1 px</option><option value="2">Normal - 2 px</option><option value="3">Thick - 3 px</option>{[4, 5, 6, 8, 10, 12].map((size) => <option key={size} value={size}>{size} px</option>)}</select></label>
