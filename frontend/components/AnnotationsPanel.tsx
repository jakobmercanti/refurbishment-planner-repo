...
export type AnnotationCommand = AnnotationToolName | "ADD_MEASUREMENT" | "REMOVE_MEASUREMENT";
export type AnnotationLineStyle = "SOLID" | "DASHED" | "DOTTED" | "SHORT_DASH" | "LONG_DASH" | "DASH_DOT" | "DASH_DOT_DOT";
export type AnnotationArrowEndStyle = "CLASSIC" | "OPEN" | "CIRCLE" | "SQUARE";
export type AnnotationStyle = { lineStyle: AnnotationLineStyle; thickness: number; textSize: number; rotation?: number; color?: string; arrowEndStyle?: AnnotationArrowEndStyle };
...
interface AnnotationsPanelProps {
  ...
  onCommandDoubleClick: (command: AnnotationCommand) => void;
...
const toolButton = (current: AnnotationToolName | null, value: AnnotationToolName, onToolChange: (tool: AnnotationToolName) => void, onCommandDoubleClick: (command: AnnotationCommand) => void, label: string) => (
  <button type="button" data-repeat-command={"ANNOTATION:" + value} className={current === value ? "active" : ""} aria-pressed={current === value} onClick={() => onToolChange(value)} onDoubleClick={() => onCommandDoubleClick(value)}>{label}</button>
);

export function AnnotationsPanel({ tool, style, selection, onToolChange, onCommandDoubleClick, onStyleChange, onTextChange, onDeleteSelected, measurementMode, onAddMeasurement, onRemoveMeasurement, onClearToolSelection }: AnnotationsPanelProps) {
  return <div className="annotations-panel" onPointerDown={(event) => {
    ...
    <div className="annotation-panel-group">
      ...
      <div className="annotation-tool-grid">
        {toolButton(tool, "LINE", onToolChange, onCommandDoubleClick, "Line")}
        {toolButton(tool, "ARROW", onToolChange, onCommandDoubleClick, "Arrow")}
        {toolButton(tool, "POLYLINE", onToolChange, onCommandDoubleClick, "Polyline")}
      </div>
    </div>
    <div className="annotation-panel-group">
      <span className="annotation-panel-label">Annotate</span>
      <div className="annotation-tool-grid">
        {toolButton(tool, "TEXT", onToolChange, onCommandDoubleClick, "Text")}
        {toolButton(tool, "CALLOUT", onToolChange, onCommandDoubleClick, "Callout")}
        {toolButton(tool, "MARKER", onToolChange, onCommandDoubleClick, "Marker")}
      </div>
    </div>
    <div className="annotation-panel-group">
      <span className="annotation-panel-label">Measure</span>
      <div className="annotation-tool-grid annotation-measure-grid">
        <button type="button" data-repeat-command="ANNOTATION:MEASUREMENT:ADD" className={measurementMode === "ADD" ? "active" : ""} aria-pressed={measurementMode === "ADD"} onClick={onAddMeasurement} onDoubleClick={() => onCommandDoubleClick("ADD_MEASUREMENT")}>Add measurement</button>
        <button type="button" data-repeat-command="ANNOTATION:MEASUREMENT:REMOVE" className={measurementMode === "REMOVE" ? "active danger-button" : "danger-button"} aria-pressed={measurementMode === "REMOVE"} onClick={onRemoveMeasurement} onDoubleClick={() => onCommandDoubleClick("REMOVE_MEASUREMENT")}>Remove measurement</button>
      </div>
    ...
    <div className="annotation-panel-group annotation-style-group">
      ...
      <label className="annotation-style-row"><span>Line style</span><select value={style.lineStyle} onChange={(event) => onStyleChange({ lineStyle: event.target.value as AnnotationLineStyle })}><option value="SOLID">Solid</option><option value="DASHED">Dashed</option><option value="DOTTED">Dotted</option><option value="SHORT_DASH">Short dash</option><option value="LONG_DASH">Long dash</option><option value="DASH_DOT">Dash-dot</option><option value="DASH_DOT_DOT">Dash-dot-dot</option></select></label>
      <label className="annotation-style-row"><span>Thickness</span><select value={style.thickness} onChange={(event) => onStyleChange({ thickness: Number(event.target.value) })}>{[1, 2, 3, 4, 5, 6, 8, 10, 12].map((size) => <option key={size} value={size}>{size} px</option>)}</select></label>
      <label className="annotation-style-row"><span>Text size</span><select value={style.textSize} onChange={(event) => onStyleChange({ textSize: Number(event.target.value) })}>{Array.from(new Set([8, 10, 12, 14, 16, 18, 20, 22, 24, 28, 32, 36, 40, 48, 56, 64, 72, 96, style.textSize])).sort((a, b) => a - b).map((size) => <option key={size} value={size}>{size} px</option>)}</select></label><label className="annotation-style-row"><span>Rotation (degrees)</span><input aria-label="Text rotation in degrees" type="number" min="-180" max="359" step="1" value={style.rotation ?? 0} onChange={(event) => { const value = Number(event.target.value); if (Number.isFinite(value)) onStyleChange({ rotation: Math.max(-180, Math.min(359, value)) }); }} /></label><label className="annotation-style-row"><span>Colour</span><input aria-label="Annotation colour" type="color" value={style.color ?? "#287fb8"} onChange={(event) => onStyleChange({ color: event.target.value })} /></label>{selection?.type === "ARROW" && <div className="annotation-style-row"><span>Arrow end style</span><ArrowEndStyleSelect value={style.arrowEndStyle ?? "CLASSIC"} onChange={(arrowEndStyle) => onStyleChange({ arrowEndStyle })} /></div>}
    </div>
