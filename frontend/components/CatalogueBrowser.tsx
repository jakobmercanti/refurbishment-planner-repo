...
export function CatalogueBrowser({ apiUrl, open, displayUnits, onClose, onInsert }: CatalogueBrowserProps) {
  ...
  function renderDetailDialog() {
    ...
    if (detailEntry.type === "object") {
      ...
      const preview = item.images?.[0]?.data_url || (item.representation_key ? `/fixture-previews/${item.representation_key}.${(/^(living|bedroom)-/.test(item.category_id) || BATH_KEYS.includes(item.representation_key) || CABINET_KEYS.includes(item.representation_key)) ? "png" : /^(living|bedroom|kitchen|radiators|staircases)-/.test(item.category_id) ? "svg" : "png"}` : undefined);
