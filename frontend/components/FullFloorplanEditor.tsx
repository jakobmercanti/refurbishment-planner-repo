...
.plan-grid{display:none}.canvas-background{fill:#fff}.wall-body,.wall-line{stroke:#151515}.wall-line{stroke-width:var(--wall-inner-stroke-width,5px)}.wall-thickness-label{fill:#151515}.vertex-layer{opacity:0}.vertex-label{display:none}.opening-dimension{display:none}.full-room-highlight polygon{display:none}.export-room-name{fill:#151515;font-family:Arial,sans-serif;font-size:9px;font-weight:700;letter-spacing:.02em}.opening-gap{stroke:#fff;stroke-width:12}.opening-jamb{stroke:#151515;stroke-width:1.4}.door-closed-line{display:none}.door-leaf{stroke:#606060;stroke-width:1.15}.door-swing{stroke:#777;stroke-width:1;stroke-dasharray:none}.window-frame{stroke:#151515;stroke-width:1.25}.window-core{stroke:#777;stroke-width:.8}.window-jamb{stroke:#151515;stroke-width:1.2}`;
...
export function FullFloorplanEditor({ apiUrl, displayUnits, floorplanStyle, exportRequest, activeRoomName, fixtures = [], onFixturesChange, onOpenRoom, toolbarVisibility, onToggleToolbar, toolbarLayoutResetKey }: Props) {
  ...
  function setSelectedWallThickness(wallId: string, segmentIndex: number, thicknessMm: number) {
    ...
    const currentThickness = wallThicknessForSegment(walls.find((wall) => wall.id === wallId) ?? { id: "", points: [] }, segmentIndex, wallThickness);
    if (Math.abs(currentThickness - thicknessMm) <= 1e-6) return;
    record();
    ...
    <div className="editor-layout full-plan-layout">
      <aside className="editor-tools full-plan-controls">
        ...
        <section className="tool-section"><div className="tool-heading"><span>1</span><h2>Build floorplan</h2></div>
          ...
          {tool === "SELECT" && selectedSegment && selectedWall && <div className="selected-properties"><div className="tool-heading"><span>W{walls.findIndex((wall) => wall.id === selectedWall.id) + 1}.{selectedSegment.segmentIndex + 1}</span><h2>Selected wall</h2></div><p className="tool-note">Set a wall-specific thickness here, or keep the overall thickness as its default. An override is labelled directly on the drawing and included in 3D and saved room geometry.</p><label className="field"><span>New length <small>{UNIT_LABEL[displayUnits]}</small></span><DisplayNumberInput minMm={1} valueMm={wallLengthInput ?? wallLengthForSegment(selectedWall, selectedSegment.segmentIndex)} units={displayUnits} onMmChange={setWallLengthInput} /></label><button className="primary-small" onClick={applySelectedWallLength}>Apply wall length</button><label className="field"><span>Wall thickness <small>{UNIT_LABEL[displayUnits]}</small></span><DisplayNumberInput minMm={1} maxMm={2000} valueMm={wallThicknessForSegment(selectedWall, selectedSegment.segmentIndex, wallThickness)} units={displayUnits} onMmChange={(value) => setSelectedWallThickness(selectedWall.id, selectedSegment.segmentIndex, value)} /></label><small>{selectedWall.thicknessOverridesMm?.[selectedSegment.segmentIndex] === undefined ? `Using overall default: ${formatLength(wallThickness, displayUnits)}.` : `Override: ${formatLength(wallThicknessForSegment(selectedWall, selectedSegment.segmentIndex, wallThickness), displayUnits)}.`}</small><button type="button" disabled={selectedWall.thicknessOverridesMm?.[selectedSegment.segmentIndex] === undefined} onClick={() => resetSelectedWallThickness(selectedWall.id, selectedSegment.segmentIndex)}>Use overall thickness</button><div className="button-grid"><button onClick={insertPoint}>Add point at midpoint</button><button className="danger-button" onClick={() => removeSegment(selectedWall.id, selectedSegment.segmentIndex)}>Remove wall segment</button></div></div>}
      ...
      <aside className="coordinate-panel full-plan-side-column">
        ...
        <section className="tool-section"><div className="tool-heading"><span>4</span><h2>Coordinates</h2></div><p className="tool-note">{coordinateEntries.length ? "Each fixed corner label matches the numbered point on the drawing. Edit only the X and Y values." : "Add a wall run to edit its numbered corner coordinates."}</p><div className="coordinate-input-list" aria-label={`Floorplan coordinates in ${UNIT_LABEL[displayUnits]}`}>{coordinateEntries.map(({ wall, point, pointIndex, cornerNumber }) => <div key={`${wall.id}-coordinate-${pointIndex}`}><span className="coordinate-prefix">{cornerNumber} -</span><DisplayNumberInput aria-label={`Corner ${cornerNumber} X coordinate`} valueMm={point.x} units={displayUnits} onMmChange={(value) => updateCoordinatePoint(wall.id, pointIndex, { ...point, x: value })} /><span className="coordinate-comma">,</span><DisplayNumberInput aria-label={`Corner ${cornerNumber} Y coordinate`} valueMm={point.y} units={displayUnits} onMmChange={(value) => updateCoordinatePoint(wall.id, pointIndex, { ...point, y: value })} /></div>)}</div></section>
        </FloatingToolbar>}
