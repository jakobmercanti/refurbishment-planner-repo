...
export function FullFloorplanEditor({ annotateRequest = 0, onPlacementWallsChange, placement, onBeginPlacement, onCancelPlacement, onCommitPlacement, onTransferObstacle, viewerOpeningRoom, onStandaloneRoomChange, openingEditRequest, elementEditRequest, onElementSelected, openingEditorTarget, projectRooms = [], onPlanRoomChange, onPlanRoomsChange, apiUrl, displayUnits, floorplanStyle, exportRequest, importFile, activeSourceRoomId, fixtures: currentFixtures = [], onFixturesChange: currentOnFixturesChange, toolbarVisibility, onToggleToolbar, toolbarLayoutResetKey, fillToolbarLayout }: Props) {
  ...
  function applySelectedWallLength() {
    ...
    updateCoordinatePoint(selectedWall.id, endIndex, next, { segmentIndex: selectedSegment.segmentIndex, lengthMm: requested }, { allowSubMinimumLength: true });
  ...
  function updateCoordinatePoint(wallId: string, pointIndex: number, next: Point2D, lengthOverride?: { segmentIndex: number; lengthMm: number }, options?: { allowSubMinimumLength?: boolean }) {
    const raw = snapshot();
    const before = { ...raw, walls: assignStableCornerNumbers(raw.walls, wallVertexStarts) };
    const constrained = constrainCornerMoveForOpenings(before.walls, before.openings, { wallId, pointIndex }, next, options);
  ...
  return <section ref={editorRoot} className={`editor-page full-plan-page ${floorplanStyleClass(floorplanStyle)}`.trim()}>
    ...
    {annotationPanelOpen && <FloatingToolbar title="Annotations & lines" defaultPosition={{ x: 300, y: 76 }} maxHeight={560} bringToFront onClose={() => { setAnnotationPanelOpen(false); cancelAnnotationMode(); }}><AnnotationsPanel tool={annotationTool} style={annotationStyle} selection={annotationPanelSelection} measurementMode={annotationMeasurementMode} onToolChange={handleAnnotationToolChange} onStyleChange={updateAnnotationStyle} onTextChange={updateSelectedAnnotationText} onDeleteSelected={deleteSelectedAnnotation} onAddMeasurement={() => activateAnnotationMeasurement("ADD")} onRemoveMeasurement={() => activateAnnotationMeasurement("REMOVE")} onClearToolSelection={clearAnnotationToolSelection} /></FloatingToolbar>}
          ...
          <div className={`full-plan-overall-properties${overallPropertiesExpanded ? " expanded" : ""}`}>
            ...
            {overallPropertiesExpanded && <div className="full-plan-overall-content">
              ...
              <div className="plan-constraint-controls">
                ...
                <label className="check-row square-walls-control"><input type="checkbox" checked={squaredWalls} onChange={(event) => { const next = event.target.checked; setSquaredWalls(next); if (next) { record(); setWallsRespectingMeasurements((current) => current.map((wall) => ({ ...wall, points: squareWallPoints(wall.points) })), true, true); } }} /><span>Keep walls squared</span></label>
