    ...
    <div className="editor-layout full-plan-layout">
      <aside className="editor-tools full-plan-controls">
        ...
        <section className="tool-section">
          ...
          {addRoomPanelOpen && <div className="room-action-panel"><p>Select a boundary wall to create a new room on the outside of it</p><label className="field"><span>Room depth ({UNIT_LABEL[displayUnits]})</span><DisplayNumberInput minMm={200} valueMm={roomDepthInput} units={displayUnits} onMmChange={setRoomDepthInput} /></label><button aria-label="Apply Add room" onClick={addSelectedRoom}>Add room</button></div>}
          ...
          {tool === "SELECT" && selectedSegment && selectedWall && <div className="selected-properties">
            ...
            <label className="field"><span>Select a wall and define the new wall thickness ({UNIT_LABEL[displayUnits]})</span><div className="field-action-row"><DisplayNumberInput minMm={1} maxMm={2000} valueMm={wallThicknessInput?.key === `${selectedWall.id}:${selectedSegment.segmentIndex}` ? wallThicknessInput.value : wallThicknessForSegment(selectedWall, selectedSegment.segmentIndex, wallThickness)} units={displayUnits} onMmChange={(value) => setWallThicknessInput({ key: `${selectedWall.id}:${selectedSegment.segmentIndex}`, value })} /><button onClick={() => setSelectedWallThickness(selectedWall.id, selectedSegment.segmentIndex, wallThicknessInput?.key === `${selectedWall.id}:${selectedSegment.segmentIndex}` ? wallThicknessInput.value : wallThicknessForSegment(selectedWall, selectedSegment.segmentIndex, wallThickness))}>Apply wall thickness</button></div></label>
            <button type="button" disabled={selectedWall.thicknessOverridesMm?.[selectedSegment.segmentIndex] === undefined} onClick={() => { resetSelectedWallThickness(selectedWall.id, selectedSegment.segmentIndex); setWallThicknessInput(null); }}>Use default thickness</button>
            <label className="field"><span>Default wall thickness ({UNIT_LABEL[displayUnits]})</span><div className="field-action-row"><DisplayNumberInput aria-label="Selected wall panel default wall thickness" minMm={1} maxMm={2000} valueMm={defaultWallThicknessInput ?? wallThickness} units={displayUnits} onMmChange={setDefaultWallThicknessInput} /><button aria-label="Define default wall thickness from selected wall panel" onClick={() => { record(); setWallThickness(defaultWallThicknessInput ?? wallThickness); setDefaultWallThicknessInput(null); }}>Define default wall thickness</button></div></label>
          </div>}
