...
export function CasementWindow({ representation, width: w, depth: d, height: h }: { representation: string; width: number; depth: number; height: number }) {
  ...
  function box(x: number, y: number, z: number, width: number, height: number, depth: number, colour = "#f3f0e8", metal = false) {
    return <mesh position={[x, y, z]} castShadow receiveShadow><boxGeometry args={[width, height, depth]} /><meshStandardMaterial color={colour} roughness={metal ? .24 : .36} metalness={metal ? .8 : 0} /></mesh>;
  }
  return <group>
    <mesh position={[0, 0, -frameDepth / 2]} castShadow receiveShadow><extrudeGeometry args={[outer, { depth: frameDepth, bevelEnabled: false }]} /><meshStandardMaterial color="#eeeae3" roughness={.38} /></mesh>
    {Array.from({ length: count }, (_, i) => {
      ...
      return <group key={i} position={[x, border + gap, frameDepth * .18]}>
        <mesh position={[0, 0, -frameDepth * .26]} castShadow receiveShadow><extrudeGeometry args={[sash, { depth: frameDepth * .54, bevelEnabled: true, bevelSize: 1.2 * unit, bevelThickness: 1.2 * unit, bevelSegments: 2 }]} /><meshStandardMaterial color="#f7f5ef" roughness={.29} /></mesh>
        ...
        <mesh position={[0, rail * .95, frameDepth * .08]}><extrudeGeometry args={[bead, { depth: 5 * unit, bevelEnabled: true, bevelSize: unit, bevelThickness: unit, bevelSegments: 2 }]} /><meshStandardMaterial color="#eeeae1" roughness={.36} /></mesh>
