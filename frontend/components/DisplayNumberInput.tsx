...
export function DisplayNumberInput({ valueMm, onMmChange, onInvalidValue, units, minMm, maxMm, stepMm, ...props }: DisplayNumberInputProps) {
  return <EditableNumberInput {...props} value={toDisplayInputNumber(valueMm, units)} min={minMm === undefined ? undefined : toDisplayNumber(minMm, units)} max={maxMm === undefined ? undefined : toDisplayNumber(maxMm, units)} step={stepMm === undefined ? undefined : toDisplayNumber(stepMm, units)} onValueChange={(value) => onMmChange(fromDisplayNumber(value, units))} onInvalidValue={onInvalidValue} />;
}
