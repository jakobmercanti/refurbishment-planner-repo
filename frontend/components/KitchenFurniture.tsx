...
function UpperCabinet({ representation, colour, carcassColour, hardwareColour, width, depth, height, colours }: { colours: Record<string, string>; representation: string; colour: string; carcassColour: string; hardwareColour: string; width: number; depth: number; height: number }) {
  ...
  return <group>
    ...
    {[.33, .66].map(y => <group key={y}>{box(0, height * y, -t, width - t * 2, t * .8, depth - t * 3, colours.shelves ?? carcassColour)}</group>)}
...
export function KitchenFurniture({ representation: key, colour, width, depth, height, secondaryColour = "#77736B", hardwareColour = "#B6BABB", colours = {} }: {
  colours?: Record<string, string>; representation: string; colour: string; width: number; depth: number; height: number; secondaryColour?: string; hardwareColour?: string;
}) {
  if (key.startsWith("furniture-kitchen-cabinet-")) return <UpperCabinet colours={colours} representation={key} colour={colours.fronts ?? colour} carcassColour={colours.body ?? secondaryColour} hardwareColour={colours.hardware ?? hardwareColour} width={width} depth={depth} height={height} />;
  ...
  const top = sink ? .78 : 1;
  const metal = colours.hardware ?? hardwareColour, dark = colours.display ?? "#20282D";
  const partId = (id: string) => {
    if (id === "plinth") return "plinth";
    if (id === "top") return fridge || wardrobe ? "body" : "top";
