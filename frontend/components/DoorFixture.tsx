...
function DoorLeaf({ width: w, height: h, thickness: t, style, colour, handleSide = 1, sliding = false, showHandle = true, colours = {} }: { width: number; height: number; thickness: number; style: string; colour: string; handleSide?: number; sliding?: boolean; showHandle?: boolean; colours?: Record<string, string> }) {
  const grain = useMemo(() => {
  ...
  return <group>
    ...
    {panels.map((p, index) => {
      ...
      return <group key={index} position={[x, y, 0]}>
        ...
        </> : <mesh position={[0, 0, -t * .09]} castShadow receiveShadow><extrudeGeometry args={[inset, { depth: t * .18, bevelEnabled: style !== "shaker", bevelSize: trim * .25, bevelThickness: t * .10, bevelSegments: 3 }]} /><OpeningFinishMaterial colour={colours.panels ?? colour} /></mesh>}
        {[-1, 1].map(side => <mesh key={side} position={[0, 0, side * t * .32]} rotation={[side < 0 ? Math.PI : 0, 0, 0]} castShadow receiveShadow><extrudeGeometry args={[ring, { depth: t * .14, bevelEnabled: style !== "shaker", bevelSize: trim * .4, bevelThickness: t * .10, bevelSegments: 3 }]} /><OpeningFinishMaterial colour={colours.panels ?? colour} /></mesh>)}
      </group>;
    })}
    {style === "barn" && <>
      {Array.from({ length: 7 }, (_, i) => <Box key={i} at={[-w / 2 + (i + 1) * w / 8, h / 2, t * .502]} size={[w * .003, h, t * .01]} colour="#5c5145" />)}
      {[.12, .86].map(y => <Box key={y} at={[0, y * h, t * .75]} size={[w * .9, h * .065, t * .5]} colour={colours.panels ?? colour} />)}
...
/** Frame and leaf share dimensions but have independent finish regions. */
export function DoorFixture({ representation, width, depth, height, colour = "#f4f3ee", frame = false, colours = {} }: { representation: string; width: number; depth: number; height: number; colour?: string; frame?: boolean; colours?: Record<string, string> }) {
  ...
  return <group>
    {Array.from({ length: model.leaves }, (_, i) => <group key={i} position={[-innerWidth / 2 + gap + leafWidth / 2 + i * (leafWidth + gap), gap, model.operation === "SLIDING" && model.leaves > 1 ? (i % 2 ? 1 : -1) * thickness * .6 : 0]}>
      <DoorLeaf width={leafWidth} height={leafHeight} thickness={thickness} style={model.style} colours={colours} colour={colours.leaf ?? colour} handleSide={model.leaves > 1 && i % 2 ? -1 : 1} sliding={model.operation === "SLIDING"} showHandle={model.operation !== "FOLDING" || i % 2 === 0} />
    </group>)}
    {model.operation !== "HINGED" && <Box at={[0, height - unit * 8, 0]} size={[width, unit * 14, thickness * 1.4]} colour={colours.track ?? "#5a6061"} metal />}
    {frame && <>
      {[-1, 1].map(side => <group key={side}>
        <Box at={[side * (width - jamb) / 2, height / 2, 0]} size={[jamb, height, depth]} colour={colours.frame ?? colour} />
        <Box at={[side * (width / 2 - jamb - unit * 5), height / 2, -thickness * .65]} size={[unit * 10, height - jamb, unit * 14]} colour={colours.frame ?? colour} />
      </group>)}
      <Box at={[0, height - jamb / 2, 0]} size={[width, jamb, depth]} colour={colours.frame ?? colour} />
      {[-1, 1].map(face => <group key={face}>
        {[0, 1, 2].map(layer => {
          ...
          return <mesh key={layer} position={[0, 0, projection]} rotation={[0, face < 0 ? Math.PI : 0, 0]} castShadow receiveShadow><extrudeGeometry args={[profile, { depth: unit * 8, bevelEnabled: true, bevelSize: unit * .8, bevelThickness: unit * .8, bevelSegments: 2 }]} /><OpeningFinishMaterial colour={colours.frame ?? colour} /></mesh>;
        })}
      </group>)}
    </>}
  </group>;
