...
export function Popup({ open, title, message, children, className = "", confirmLabel = "Ok", confirmDisabled = false, onConfirm, onCancel }: PopupProps) {
  if (!open) return null;
  return <div className="popup-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onCancel(); }}>
    <section className={`popup ${className}`.trim()} role="dialog" aria-modal="true" aria-labelledby="popup-title" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <footer className="popup-actions"><button type="button" onClick={onCancel} autoFocus>Cancel</button><button type="button" className="primary" disabled={confirmDisabled} onClick={onConfirm}>{confirmLabel}</button></footer>
    </section>
  </div>;
}
