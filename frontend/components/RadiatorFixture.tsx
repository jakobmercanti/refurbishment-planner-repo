...
export function RadiatorFixture({ representation, width: w, depth: d, height: h, colour = "#FFFFFF", colours = {} }: { colours?: Record<string, string>; representation: string; width: number; depth: number; height: number; colour?: string }) {
  ...
  return <group>
    ...
    {[-1, 1].map(side => <group key={side}>
      ...
      <mesh position={[side * w * .43, h * .025, d * .23]} castShadow><cylinderGeometry args={[radius * 1.7, radius * 1.7, radius * 2.5, 16]} /><meshStandardMaterial color={colours.caps ?? "#eeeeeb"} roughness={.38} /></mesh>
      {[.23, .77].map(y => <mesh key={y} position={[side * w * .32, h * y, -d * .42]} castShadow><boxGeometry args={[radius * 2, radius * 4, d * .15]} /><meshStandardMaterial color={colours.brackets ?? colour} roughness={.4} /></mesh>)}
    </group>)}
  </group>;
}
