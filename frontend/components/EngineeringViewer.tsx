...
function WallPiece({
  ...
  useFrame(({ camera }) => {
    ...
    let isVisible = true;
    if (wallMode !== "CUTAWAY_2D" && wallMode !== "CUTAWAY_3D") {
      isVisible = true;
    } else {
...
function WheelZoom({ onManualViewChange }: { onManualViewChange?: () => void }) {
  ...
  useEffect(() => {
    ...
    eventSource.addEventListener("wheel", zoomWithWheel, { passive: false });
    return () => eventSource.removeEventListener("wheel", zoomWithWheel);
  }, [camera, events, onManualViewChange]);
  return null;
}
...
function PlacementCursor({ request, rooms, walls, onCommit, onCancel, onPreviewCandidate }: { request: PlacementRequest; rooms: Room[]; walls: PlacementWall[]; onCommit?: (candidate: PlacementCandidate)=>void; onCancel?: ()=>void; onPreviewCandidate?: (candidate: PlacementCandidate | null) => void }) {
  ...
  },[camera,events,raycaster,request.opening,rooms]);
  ...
  useEffect(()=>{
    ...
    const press=(event:PointerEvent)=>{
      ...
      if (event.button===0) { down={x:event.clientX,y:event.clientY}; eventSource.setPointerCapture(event.pointerId); }
    };
    const release=(event:PointerEvent)=>{
      event.preventDefault(); event.stopImmediatePropagation();
      if (eventSource.hasPointerCapture(event.pointerId)) eventSource.releasePointerCapture(event.pointerId);
      ...
      if (next && (!next.roomId || rooms.some(room=>room.id===next.roomId))) {
        ...
        eventSource.addEventListener("click",swallow,{capture:true,once:true});
        window.setTimeout(()=>eventSource.removeEventListener("click",swallow,true),0);
        onCommit?.(next);
      }
    };
    ...
    eventSource.addEventListener("pointermove",move,true); eventSource.addEventListener("pointerdown",press,true);
  ...
  },[events,pointAt,request,rooms,walls,onCommit,onCancel]);
...
function Scene({ placementWalls = [], placement, onCommitPlacement, onCancelPlacement, onTransferObstacle, room, sceneRooms, collisionIds, onObstaclesChange, onPersonChange, onRenderCameraChange, renderCamera, wallMode, toggles, preset, projection, selection, onSelectionChange, showGrid, cameraResetKey, fitRequest, fitViewRequest, zoomPercent, lighting, onManualViewChange, restoreView, cameraStateRef, onCameraViewRestored, onFitComplete, cameraRigVisible, selectedCameraRigHandle, onCameraRigHandleSelect }: ViewerProps & {
  ...
  useEffect(() => {
    ...
    const eventSource=events.connected;
    if (!eventSource) return;
    eventSource.addEventListener("pointercancel",cancel);
