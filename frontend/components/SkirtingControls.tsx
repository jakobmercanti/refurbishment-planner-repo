...
export function SkirtingControls({ value, onChange, collapsed = false, onToggleCollapsed }: { value?: SkirtingBoardSettings; onChange: (settings: SkirtingBoardSettings) => void; collapsed?: boolean; onToggleCollapsed?: (collapsed: boolean) => void }) {
  ...
  return <section data-skirting-controls style={{ borderTop: "1px solid #c6c9c4", marginTop: 16, paddingTop: 12 }}>
    ...
    <div className="skirting-controls-heading"><label className="viewer-lock-choice"><input type="checkbox" checked={settings.enabled} aria-controls={id} aria-expanded={settings.enabled} onChange={event => update({ enabled: event.target.checked })} /><span>Skirting board</span></label>{settings.enabled && <button type="button" className="skirting-summary-action skirting-hide-button" aria-label="Hide skirting board settings" aria-expanded="true" onClick={() => onToggleCollapsed?.(true)}>Hide settings</button>}</div>
