...
export default function BillingPage() {
  ...
  useEffect(() => {
    ...
    void Promise.all([commercialRequest<Catalogue>("/catalogue", {}, false), currentSession()]).then(async ([products, signedIn]) => {
      if (!mounted) return;
      setCatalogue(products); setSession(signedIn);
      if (signedIn) {
        try { setSummary(await commercialRequest<Summary>("/summary")); } catch { /* account page explains if services are not configured */ }
      }
  ...
  async function checkout(key: string) {
    ...
    try {
      if (!session) { router.push(`${base}/account/`); return; }
  ...
  return <main className="commercial-page">
    <header className="commercial-header"><a className="commercial-brand" href={`${base}/`}>FreeFloorplan3D</a><nav><a href={`${base}/`}>Planner</a><a href={`${base}/workspace/`}>Workspace</a>{session ? <button type="button" onClick={() => void portal()} disabled={busy}>Manage billing</button> : <a href={`${base}/account/`}>Sign in</a>}</nav></header>
    <div className="commercial-content"><section className="commercial-page-heading"><div><p className="commercial-eyebrow">PLANS & BILLING</p><h1>Keep planning free. Add cloud when you need it.</h1><p>The 2D/3D planner stays usable without an account. Paid plans add private cloud storage, project backup and optional visual renders.</p></div></section>
