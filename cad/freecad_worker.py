...
def main() -> None:
    ...
    wall_thickness_overrides = room.get("wall_thickness_overrides_mm", {})
    wall_shapes: dict[str, object] = {}
    for index, start in enumerate(vertices):
        end = vertices[(index + 1) % len(vertices)]
        wall_id = f"wall-{index + 1:03d}"
        segment_thickness = float(wall_thickness_overrides.get(wall_id, wall_thickness))
        _, _, exterior_x, exterior_y, _ = edge_geometry(start, end)
        points = [
            vector(start),
            vector(end),
            App.Vector(float(end["x"]) + exterior_x * segment_thickness, float(end["y"]) + exterior_y * segment_thickness, 0),
            App.Vector(
                float(start["x"]) + exterior_x * segment_thickness, float(start["y"]) + exterior_y * segment_thickness, 0
            ),
        ]
        ...
        for opening in room.get("openings", []):
            if opening["parent_wall_id"] == wall_id:
                cut = opening_cut_shape(start, end, opening, segment_thickness)
                shape = shape.cut(cut)
        ...
        wall_object.Thickness = segment_thickness
